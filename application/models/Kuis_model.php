...
class Kuis_model extends CI_Model
{
	...
	private $table_quiz_questions = 'quiz_questions';
  ...
  private $table_answers = 'quiz_answers';
	...
	public function update_quiz($quiz_id, $data) { return $this->db->where('quiz_id', $quiz_id)->update($this->table_quizzes, $data); }
	public function delete_quiz($quiz_id) { return $this->db->where('quiz_id', $quiz_id)->delete($this->table_quizzes); }

	public function get_quiz_by_id($quiz_id)
	{
		return $this->db->where('quiz_id', $quiz_id)->get($this->table_quizzes)->row();
	}

	public function get_questions_by_id($id)
	{
		return $this->db->where('question_id', $id)->get($this->table_quiz_questions)->row();
	}
	...
	{
		return $this->db->where('question_id', $id)->update($this->table_quiz_questions, $data);
	}

	public function delete_quiz_question($id)
	{
		return $this->db->where('question_id', $id)->delete($this->table_quiz_questions);
	}
    ...
    {
        $this->db->select('question_id, quiz_id, question_text, option_a, option_b, option_c, option_d');
    ...
    public function get_quiz_review($quiz_id, $user_id)
    {
        ...
        $this->db->join($this->table_answers . ' a', 'a.question_id = q.question_id');
        $this->db->where('a.result_id', $result->result_id);
    ...
    public function submit_answers($quiz_id, $user_id, $answers)
    {
        ...
        foreach ($questions as $q) {
            $user_ans = isset($answers[$q->question_id]) ? $answers[$q->question_id] : null;
...
/* End of file Kuis_model.php */
/* Location: ./application/models/Kuis_model.php */