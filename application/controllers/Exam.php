...
class Exam extends CI_Controller
{
   ...
   * ========================= */
    public function index()
    {
        ...
        $user_id = $user['user_id']; // Ini ID Admin

        // Redirect Siswa jika salah masuk
        if ($user['role'] === 'Siswa') {
        ...
        // 2. Ambil Data Kelas milik Admin tersebut (user_id di table classes)
        $data = [
            'title'      => 'Pilih Kelas Ujian',
            'user'       => $user,
            'kelas_list' => $this->Exam_model->get_admin_classes($user_id), // Query baru dari Exam_model
            'url_name'   => 'admin' // URL Helper disesuaikan
        ];

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('exam/select_class', $data);
   ...
   * ========================= */
    public function management($class_id = null)
    {
        ...
        // 1. Validasi Admin
        if ($user['role'] !== 'Admin') {
            show_error('Akses ditolak. Khusus Admin.', 403);
        }

        // 2. Validasi Kepemilikan Kelas oleh Admin
        // user_id session Admin dicocokkan dengan user_id di table classes
        if (!$this->Exam_model->is_admin_class($class_id, $user['user_id'])) {
   ...
   * ========================= */
    public function save()
    {
        ...
        $new_date = new DateTime($start_time);
        ...
        // 3. Validasi Kepemilikan Kelas (Admin)
        $user = $this->session->userdata();
        if ($user['role'] !== 'Admin' || !$this->Exam_model->is_admin_class($class_id, $user['user_id'])) {
            echo json_encode(['status' => 'error', 'message' => 'Akses ditolak', 'csrf_hash' => $this->security->get_csrf_hash()]);
            return;
        }

        // ============================================================
        // 4. VALIDASI LOGIKA BISNIS (UTS/UAS INTERVAL) - TETAP SAMA
        // ============================================================
    ...
    public function delete()
    {
        ...
        // Validasi tambahan (Ownership) disarankan, tapi untuk mempersingkat kita andalkan User Role
        $user = $this->session->userdata();
        if ($user['role'] !== 'Admin') {
             echo json_encode(['status' => 'error', 'message' => 'Akses ditolak', 'csrf_hash' => $this->security->get_csrf_hash()]);
             return;
    ...
    public function questions($exam_id = null)
    {
        ...
        // Validasi kepemilikan kelas oleh Admin
        if (!$this->Exam_model->is_admin_class($exam->class_id, $user['user_id'])) {
            show_error('Akses ditolak', 403);
        }
    ...
    public function save_question()
    {
        ...
        // Validasi hak akses admin bisa ditambahkan di sini jika perlu keamanan ekstra untuk endpoint AJAX
    ...
    public function save_questions_batch()
    {
        ...
        if (!$exam || !$this->Exam_model->is_admin_class($exam->class_id, $user['user_id'])) {
            echo json_encode(['status' => 'error', 'message' => 'Akses ditolak']);
            return;
        }
   ...
   * ========================= */
    public function result($exam_id = null)
    {
        ...
        $user = $this->session->userdata();
        // Validasi Admin
        if ($user['role'] !== 'Admin') show_error('Akses ditolak', 403);

        $exam = $this->Exam_model->get_exam_with_class($exam_id);
        
        // Validasi kepemilikan kelas (opsional tapi disarankan)
        if (!$this->Exam_model->is_admin_class($exam->class_id, $user['user_id'])) {
             show_error('Akses ditolak', 403);
        }
        ...
        if ($count_finished == 0) $min_score = 0;
    ...
    public function export_result($exam_id = null)
    {
        ...
        // Validasi Admin (Sederhana via session)
        ...
        $no = 1;
        foreach ($students as $s) {
            $status = ($s->attempt_status == 'finished') ? 'Selesai' : (($s->attempt_status == 'ongoing') ? 'Mengerjakan' : 'Belum');

            $score = ($s->score !== null) ? str_replace('.', ',', $s->score) : '0';
